...
1、不要翻译dom元素,dom元素的数量和格式不要改变。
2、表格的所有样式都需要保留，表格的行数和列数必须保持一致，如果有合并的行或者列，需要保持原有的合并状态。
3、请注意翻译时数字的格式。
4、一个p元素里面有多个span的时候，这多个span元素都要保留。
5、children为空的dom元素都要保留。
6、所有dom元素不要合并，保留和我传递给你一样的dom结构，尤其重要，必须要一致，一定要一致，如果这一条不能保证，那你生成的就不能用。
